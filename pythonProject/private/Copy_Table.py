...
# 복사할 테이블 이름 리스트
table_names = ['TABLE_NAME']  # 필요한 테이블 이름들을 여기에 추가

for table_name in table_names:
    # 소스 스키마에서 테이블 구조 가져오기
    source_cursor.execute(f"SELECT column_name, data_type, data_length, data_precision, data_scale FROM user_tab_columns WHERE table_name = '{table_name.upper()}'")
    ...
    for index_name, columns in indexes.items():
        create_index_sql = f"CREATE INDEX {index_name} ON {table_name} ({', '.join(columns)})"
        try:
    ...
    # 소스 DB에서 테이블 데이터 가져오기
    source_cursor.execute(f"SELECT {columns_str} FROM {table_name}")
    rows = source_cursor.fetchall()

    # 데이터 삽입
    insert_query = f'INSERT INTO {table_name} ({columns_str}) VALUES ({", ".join([":" + str(i+1) for i in range(len(columns))])})'
    for row in rows:
        target_cursor.execute(insert_query, row)
    print(f"Data copied successfully for {table_name}.")
